...
- **Approximate numeric aggregation**: When the KB itself provides ranges or approximate durations, accept a grounded range or nearby midpoint estimate. Do not require exact arithmetic phrasing if the assistant's number falls within a reasonable tolerance band for the same underlying facts.
- **Hedged false-presupposition correction**: Treat phrasings like "I don't recall you mentioning that," "I don't have any record of that," and "you haven't mentioned that" as semantically equivalent corrections when they all reject the same false prior claim.
- **Embedded misinformation correction**: If the assistant clearly states the corrected fact, do not fail merely because it does not mirror or exhaustively restate every part of the user's incorrect claim in the same format.
